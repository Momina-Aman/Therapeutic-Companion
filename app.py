...
if __name__ == "__main__":
    ...
    # Verify directory structure and database
    try:
        # Check if database exists
        db_path = "therapeutic_companion.db"
        if os.path.exists(db_path):
            print("✅ [TEST] Database file exists:", db_path)
        else:
            print("⚠️  [TEST] Database file not found at:", db_path)

        # Check if user_data directory exists
        user_data_dir = Path("./user_data")
        if user_data_dir.exists():
            print("✅ [TEST] User data directory exists:", user_data_dir)
        else:
            print("⚠️  [TEST] User data directory not found. Will be created on first registration.")
        ...
        if table_exists:
            print("✅ [TEST] Users table exists in database")
        else:
            print("❌ [TEST] Users table not found in database")

        print("\n🎉 [TEST DEPLOYMENT] Therapeutic Companion Phase 1 initialized successfully!")

    except Exception as e:
        print(f"❌ [TEST] Error during deployment verification: {e}")
